...
* It's using a MySQL as database. Just because it's the most popular one. Choose to use a relational just because it's a general application type of database.
  Note: In order to scale it should use a CQRS architecture with a most suitable NoSQL database to query, like Redis/MongoDB/ElasticSearch.
* It's using H2 database in MySQL compatibility mode in Unit Tests and also when running the application outside a container locally.
  Suggestion: Even though this way the tests run faster and consuming less memory in the pipelines, it is advisable to use a solution like Testcontainers to use a real MySQL database in order to be sure that there is no incompatibilities or false positives.
* Using raw SQL to represent database schema.
  Suggestion: It should be evolved to a more robust solution like Liquibase or Flyway to keep track of database schema evolution.
...
* Users are being store in the same database. Tokens JWT are being generated by a login endpoint in the same service just for simplicity.
...
* For tests, using JUnit and Mockito, with the addition to BDDMockito to be more compliant with the GivenWhenThen style, and AssertJ for fluent assertions. Leveraging JUnit `@Nested` to create independent classes by each feature, what improves organization and maintainability.
* Regarding design patters, I used extensively the Builder with aid of Lombok. As the API didn't have any complex business rules, I didn't see any opportunity to use patterns like Strategy, Chain of Responsibility, Decorator, Observer or State.
* Using the Domain Driven Design approach, that is almost a norm in Spring. Also incorporate some Clean Architecture principles, as the use DTOs between all layers. Repositories uses Entities, Services uses DTOs, Controllers uses API Request/Response. Also using Clean Code principles with code readability in mind.
* Using the most I can in java of functional programming, with immutable objects in all the layers, except by the JPA entities that must be mutable, streams and lambdas
* Using traditional Spring MVC blocking / non-reactive controllers as this API is basically for database access, that must use JDBC blocking operations anyway. So using WebFlux would bring more complexity without improving performance.
...
The package has the Swagger UI embedded, so you can see the docs and also test the endpoints.
...
```
Use the /api/v1/login endpoint to login. Available also in the Swagger UI.

After getting the token, to use it in the authenticated endpoints in Swagger UI is needed to click in the Authorize button on top of the page. All configurations are done to send it automatically as a http header authorization.
